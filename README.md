...
11	Lambda 表达式。
	11.1	标识符匿名函数 。
		11.1.1	 =值传递。
		11.1.2	&引用传递。
	11.2	 ()参数 ，意思是前面信号传递的参数，在这里使用了那些要表明。
	11.3	 {}实现体。
...
1 	QMainWindow
	...
	1.3	状态栏最多一个
		...
		1.3.2	设置到窗口中setStatusBar(stBar);
...
4	界面布局
	4.1	实现登陆窗口。
	4.2	利用布局方式给窗口进行美化。
	4.3 	选取widget 进行布局,水平布局、垂直布局、栅格布局。
	4.4	给用户名、密码、登陆、退出按钮进行布局。
	4.5	默认窗口和控件之间有间隙，可以调整layoutLefMargine
	4.6	利用弹簧进行布局
5	控件
	5.1	按钮组
		5.1.1 	QPushButton 常用按钮。
		5.1.2	QToolButton 工具按钮用于显示图片， 如图想显示文字，修改风格:
			toolButtonStyle，凸起风格 autoRaiser
		5.1.3 	radioButton 单选按钮， 设置默认u->rBtnMan->setCheckedltrue); 
		5.1.4 	checkbox 多选按钮，监听状态,
	5.2	QLstWidget 列表容器+
		5.2.1 	QListWidgetitem* item - -行内容。
		5.2.2 	ui->listWidget ->aditem ( item )
		5.2.3	设置居中方式te->stTetlgmet(Qt::AlignHCenter);
		5.2.4	可以利用additems.-次性添加整 个诗内容
	5.3	QTreWidget树控件+
		5.3.1	设置头+
			5.3.1.1	ui->treeWidget->setHeaderLabels(QStringList)<< "英雄"<< "英雄介绍");
		5.3.2	创建根节点。
			5.3.2.1 	QTreeWldgetitem *litem = new Treeidettem(QStringist)<<力量");
		5.3.3	添加根节点到树控件上
			5.3.3.1 	ui->treeWidget->addToplevelltem(item);
		5.3.4	添加子节点
			5.3.4.1 	litem->addChild(1);
	5.4 	QTableWidget 表格控件
		5.4.1	设置列数
			5.4.1.1 	ui->tableWidget->setColumnCount(3);+
		5.4.2	设置水平表头，
			5.4.2.1 	ui->tableWdget->setHorizontalHeader. abels(QStringList)<<姓名"<<性
			别"<<“年龄");
		5.4.3	设置行数
			5.4.3.1 	ui->tableWidget- >setRowCount(5);
		5.4.4	设置正文
			5.4.4.1 	ui->tableWidget->setltem(0,0, new QTableWidgtltem"亚瑟));
	5.5	其他控件介绍
		5.5.1 	stackedWidget栈控件
			5.5.1.1 	ui->stackedWidget->setCurentIndex(1)
		5.5.2	下拉框
			5.5.2.1 	ui->comboBox->addltem(奔驰")
		5.5.3 	QLabel 显示图片
			5.5.3.1 	ui->Ibl. Jmage->setPixmap(QPixmap(":/mag/buttrflygng")
		5.5.4 	QLabel 显示动图gif 图片
